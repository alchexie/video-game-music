...
所有 `/api/admin/*` 接口需在请求头中携带管理员令牌：

```
X-Admin-Token: <your-admin-token>
```

若 `adminToken` 未配置，则所有管理接口**不进行鉴权验证**。

---

### POST /api/admin/import/commit

触发媒体库扫描与导入，将磁盘上的音频文件同步到数据库。

**无请求体**

#### 响应

返回导入汇总信息（`LibraryScanSummary`）。
...
### POST /api/admin/sync/cos

将本地媒体文件同步上传至腾讯云对象存储（COS）。
...
修改合集信息。请求体与 `POST /api/admin/collections` 相同，所有字段均为可选。
...
### POST /api/admin/collections/:id/tracks
